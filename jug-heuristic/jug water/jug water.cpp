...
int main()
{
	...
	while (selectedState.first.GetValue()!=tempRestValueRequired)
	{
		...
		//Add actions 
		if (!contains(ExpandableList, selectedState.EmptyFirstJug()) && !contains(CheckedList, selectedState.EmptyFirstJug()))
		{
			ExpandableList.push_back(selectedState.EmptyFirstJug());
		}
		if (!contains(ExpandableList, selectedState.EmptySecondJug()) && !contains(CheckedList, selectedState.EmptySecondJug()))
		{
			ExpandableList.push_back(selectedState.EmptySecondJug());
		}
		if (!contains(ExpandableList, selectedState.FillFirstJug()) && !contains(CheckedList, selectedState.FillFirstJug()))
		{
			ExpandableList.push_back(selectedState.FillFirstJug());
		}
		if (!contains(ExpandableList, selectedState.FillSecondJug()) && !contains(CheckedList, selectedState.FillSecondJug()))
		{
			ExpandableList.push_back(selectedState.FillSecondJug());
		}
		if (!contains(ExpandableList, selectedState.PourToFirstJug()) && !contains(CheckedList, selectedState.PourToFirstJug()))
		{
			ExpandableList.push_back(selectedState.PourToFirstJug());
		}
		if (!contains(ExpandableList, selectedState.PourToSecondJug()) && !contains(CheckedList, selectedState.PourToSecondJug()))
		{
			ExpandableList.push_back(selectedState.PourToSecondJug());
		}
		
		//sort expandable nodes according to the nodes total cost
		for (size_t i = 0; i < ExpandableList.size(); i++)
		{
			if (ExpandableList.at(0).cost + heuristicCost(ExpandableList.at(0), tempRestValueRequired) > ExpandableList.at(i).cost + heuristicCost(ExpandableList.at(i), tempRestValueRequired))
			{
				//change elements locations
				State temp = ExpandableList.at(0);
				ExpandableList.at(0) = ExpandableList.at(i);
				ExpandableList.at(i) = temp;
			}
		}
